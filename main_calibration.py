...
from networks.resnet import SupConResNet_pl_non_nom, LinearClassifier
...
def parse_option():
    ...
    parser.add_argument('--dataset', type=str, default='cifar10',
                        choices=['cifar10', 'cifar100', 'tiny-imagenet','mix_digit'], help='dataset')
    ...
    parser.add_argument('--data_use', type=str, default="all",
                        help='all vs memory')
    parser.add_argument('--map_pos', type=str, default="embed",
                        help='embed vs logit')
...
class PL_model(pl.LightningModule):
    def __init__(self, opt, unique_cls=[]):
        super().__init__()
        self.opt = opt
        self.model = SupConResNet_pl_non_nom(name=opt.model)
        ...
        if opt.map_pos == "embed":  
            ...
            self.distill_predictor =  nn.Linear(2048, 2048)
