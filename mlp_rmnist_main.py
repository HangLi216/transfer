...
def main():
    ...
    if opt.dataset == 'rmnist':
        from networks.mlp_rmnist import SupConMLP as network
        from networks.mlp_rmnist import LinearClassifier
    else:
    ...
    for t, newtask_cls, sum_cls in taskcla: 
        ...
        # build trainer  
        trainer = pl.Trainer(max_epochs=opt.epochs, progress_bar_refresh_rate=1, callbacks=callbacks)
        
        if t > len(taskcla)-2:
            trainer_clf = pl.Trainer(max_epochs=opt.epochs)
        else:
            trainer_clf = pl.Trainer(max_epochs=opt.epochs)
