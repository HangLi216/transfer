...
def parse_option():
    ...
    parser.add_argument('--dataset', type=str, default='rmnist',
                        choices=['rmnist','cifar10', 'cifar100', 'tiny-imagenet','mix_digit','mix_office'], help='dataset')
    ...
    parser.add_argument('--k2', type=float, default=0.01)
...
class PL_model(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        if len(self.unique_cls_pre) > 0:
            features1_prev_task = torch.cat([f1, f2], dim=0)
            features1_sim = torch.div(torch.matmul(features1_prev_task, features1_prev_task.T), self.opt.k1)
            ...
            feat_old, logits_old = self.old_model(x)
            logits_old = F.normalize(logits_old, dim=1)
            with torch.no_grad():
                features2_prev_task = logits_old
                features2_sim = torch.div(torch.matmul(features2_prev_task, features2_prev_task.T), self.opt.k2)
            ...
            dis_loss = (-logits2 * torch.log(logits1)).sum(1).mean() 
        
        self.log('train_dis_loss', dis_loss)

        return sslloss + dis_loss
