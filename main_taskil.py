...
from utils2 import TwoCropTransform,random_update,set_loader,set_weighted_loader,buffer_loader,MyDataset,Pad,Convert2RGB
...
def parse_option():
    ...
    parser.add_argument('--wandb_ifuse', type=bool, default=True,
                        help='wandb_ifuse')
    ...
    parser.add_argument('--temp_dist', type=float, default=0.5, help='temperature for loss function')
    parser.add_argument('--base_temperature_dist', type=float, default=0.07, help='temperature 2 for loss function')
...
class PL_model(pl.LightningModule):
    def __init__(self, opt, unique_cls=[]):
        ...
        self.distillcriterion = SupConLoss(temperature=opt.temp_dist,base_temperature=opt.base_temperature_dist,contrast_mode="all")
...
def Task_test(model, dataloader,taskid=0,per_taskcls=2):
    pos_start,pos_end = taskid*per_taskcls, (taskid+1)*per_taskcls
    for batch in dataloader:
        x, y = batch
        x.to(model.device)
        y.to(model.device)
        output = model(x)
        output = output[:, pos_start:pos_end]
        preds = torch.argmax(output, dim=1)+taskid*per_taskcls
        acc = accuracy(preds, y) 
        return  acc
...
def main():
    ...
    taskil_acc = np.zeros((len(taskcla), len(taskcla)), dtype=np.float32)
    ...
    for t, newtask_cls, sum_cls in taskcla: 
        ...
        if t > len(taskcla)-2:
            trainer_clf = pl.Trainer(gpus=-1, max_epochs=opt.epochs, precision=16,strategy='ddp',log_every_n_steps=1,replace_sampler_ddp=False, 
                )
        else:
            trainer_clf = pl.Trainer(gpus=-1, max_epochs=opt.epochs, precision=16,strategy='ddp',log_every_n_steps=1,replace_sampler_ddp=False, 
                )
        ...
        if t>-1:
            ...
            for u in range(t + 1):
                ...
                taskil_acci = Task_test(model_clf, test_loader,taskid=u,per_taskcls=len(newtask_cls))
                taskil_acc[t,u] = taskil_acci
    ...
    print(taskil_acc)
    print("taskil ACC mean:", np.mean(taskil_acc[opt.tasknum-1]))
    if wandb_ifuse:
